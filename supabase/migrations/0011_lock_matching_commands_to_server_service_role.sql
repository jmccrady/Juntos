-- Privileged matching/state functions are server-only. Juntos server code verifies
-- the user's normal session, then calls these RPCs with the service-role client.
-- The actor ID is passed explicitly from that trusted server boundary.

grant usage on schema private to service_role;
grant select on private.driver_verification, private.vehicle_verification to service_role;
grant select on public.user_roles, public.profiles, public.driver_profiles, public.vehicles,
  public.driver_availability, public.ride_requests, public.ride_candidates, public.ride_assignments to service_role;
grant insert, update, delete on public.ride_candidates, public.ride_assignments to service_role;
grant insert on public.ride_events to service_role;
grant update on public.ride_requests to service_role;

revoke all on function public.refresh_ride_candidates(uuid) from public, anon, authenticated, service_role;
revoke all on function public.assign_ride(uuid, uuid) from public, anon, authenticated, service_role;
revoke all on function public.respond_to_ride_offer(uuid, boolean) from public, anon, authenticated, service_role;
revoke all on function public.advance_ride(uuid, public.ride_status) from public, anon, authenticated, service_role;
revoke all on function public.cancel_ride(uuid, text) from public, anon, authenticated, service_role;

drop function public.refresh_ride_candidates(uuid);
drop function public.assign_ride(uuid, uuid);
drop function public.respond_to_ride_offer(uuid, boolean);
drop function public.advance_ride(uuid, public.ride_status);
drop function public.cancel_ride(uuid, text);

create function public.refresh_ride_candidates(p_actor_id uuid, p_ride_request_id uuid)
...
begin
  ...
  if not exists (select 1 from public.user_roles ur where ur.user_id = p_actor_id and ur.role in ('dispatcher','admin')) then raise exception 'dispatcher role required'; end if;
  ...
  if not exists (select 1 from public.ride_requests rr where rr.id = p_ride_request_id and rr.pickup_region_id is not null) then
    insert into public.ride_events (ride_request_id, actor_id, event_type, old_status, new_status) values (p_ride_request_id, p_actor_id, 'candidates_refreshed', 'requested', 'requested');
    return 0;
  end if;
  with ride as (
...
create function public.assign_ride(p_actor_id uuid, p_ride_request_id uuid, p_driver_id uuid)
returns uuid language plpgsql security invoker set search_path = pg_catalog as $$
declare v_vehicle_id uuid; v_assignment_id uuid;
begin
  if p_actor_id is null then raise exception 'actor required'; end if;
  if not exists (select 1 from public.user_roles ur where ur.user_id = p_actor_id and ur.role in ('dispatcher','admin')) then raise exception 'dispatcher role required'; end if;
  ...
  insert into public.ride_assignments (ride_request_id, driver_id, vehicle_id, assigned_by, status) values (p_ride_request_id, p_driver_id, v_vehicle_id, p_actor_id, 'offered') returning id into v_assignment_id;
  ...
  insert into public.ride_events (ride_request_id, actor_id, event_type, old_status, new_status) values (p_ride_request_id, p_actor_id, 'driver_offered', 'requested', 'matched');
  return v_assignment_id;
end; $$;

create function public.respond_to_ride_offer(p_actor_id uuid, p_ride_request_id uuid, p_accept boolean)
returns public.ride_status language plpgsql security invoker set search_path = pg_catalog as $$
declare v_assignment public.ride_assignments%rowtype; v_new_status public.ride_status;
begin
  if p_actor_id is null then raise exception 'actor required'; end if;
  select * into v_assignment from public.ride_assignments where ride_request_id = p_ride_request_id and driver_id = p_actor_id and status = 'offered' for update;
  ...
  if p_accept then
    ...
    insert into public.ride_events (ride_request_id, actor_id, event_type, old_status, new_status) values (p_ride_request_id, p_actor_id, 'driver_accepted', 'matched', 'accepted');
    ...
    if not found then raise exception 'ride is no longer awaiting acceptance'; end if;
    delete from public.ride_candidates where ride_request_id = p_ride_request_id and driver_id = p_actor_id;
    insert into public.ride_events (ride_request_id, actor_id, event_type, old_status, new_status) values (p_ride_request_id, p_actor_id, 'driver_declined', 'matched', 'requested');
    v_new_status := 'requested';
  end if;
  return v_new_status;
end; $$;

create function public.advance_ride(p_actor_id uuid, p_ride_request_id uuid, p_target public.ride_status)
returns public.ride_status language plpgsql security invoker set search_path = pg_catalog as $$
declare v_current public.ride_status; v_is_dispatch boolean := false; v_is_driver boolean := false; v_event text;
begin
  if p_actor_id is null then raise exception 'actor required'; end if;
  select exists (select 1 from public.user_roles ur where ur.user_id = p_actor_id and ur.role in ('dispatcher','admin')) into v_is_dispatch;
  select exists (select 1 from public.ride_assignments ra where ra.ride_request_id = p_ride_request_id and ra.driver_id = p_actor_id and ra.status = 'accepted') into v_is_driver;
  ...
  insert into public.ride_events (ride_request_id, actor_id, event_type, old_status, new_status) values (p_ride_request_id, p_actor_id, v_event, v_current, p_target);
  return p_target;
end; $$;

create function public.cancel_ride(p_actor_id uuid, p_ride_request_id uuid, p_reason_code text default null)
returns public.ride_status language plpgsql security invoker set search_path = pg_catalog as $$
declare v_current public.ride_status; v_rider_id uuid; v_is_dispatch boolean := false;
begin
  if p_actor_id is null then raise exception 'actor required'; end if;
  ...
  if not found then raise exception 'ride not found'; end if;
  select exists (select 1 from public.user_roles ur where ur.user_id = p_actor_id and ur.role in ('dispatcher','admin')) into v_is_dispatch;
  if p_actor_id = v_rider_id then
    if v_current not in ('requested','matched','accepted') then raise exception 'rider cannot cancel ride in this state'; end if;
  elsif v_is_dispatch then
    if v_current not in ('requested','matched','accepted','arrived') then raise exception 'dispatcher cannot cancel ride in this state'; end if;
  ...
  insert into public.ride_events (ride_request_id, actor_id, event_type, old_status, new_status, reason_code) values (p_ride_request_id, p_actor_id, 'ride_cancelled', v_current, 'cancelled', p_reason_code);
  return 'cancelled';
end; $$;

revoke all on function public.refresh_ride_candidates(uuid, uuid) from public, anon, authenticated, service_role;
revoke all on function public.assign_ride(uuid, uuid, uuid) from public, anon, authenticated, service_role;
revoke all on function public.respond_to_ride_offer(uuid, uuid, boolean) from public, anon, authenticated, service_role;
revoke all on function public.advance_ride(uuid, uuid, public.ride_status) from public, anon, authenticated, service_role;
revoke all on function public.cancel_ride(uuid, uuid, text) from public, anon, authenticated, service_role;
grant execute on function public.refresh_ride_candidates(uuid, uuid) to service_role;
grant execute on function public.assign_ride(uuid, uuid, uuid) to service_role;
grant execute on function public.respond_to_ride_offer(uuid, uuid, boolean) to service_role;
grant execute on function public.advance_ride(uuid, uuid, public.ride_status) to service_role;
grant execute on function public.cancel_ride(uuid, uuid, text) to service_role;

comment on function public.refresh_ride_candidates(uuid, uuid) is 'Server-only deterministic candidate refresh. Callable only with service role; actor authorization is still validated inside the function.';
comment on function public.assign_ride(uuid, uuid, uuid) is 'Server-only dispatcher assignment. Revalidates driver eligibility and requires actor dispatcher/admin role.';
